...
int ObjMnts1::GetPassable( int icn, uint32_t index )
{
    ...
    return ARRAY_COUNT_END( disabled2 ) != std::find( disabled2, ARRAY_COUNT_END( disabled2 ), index ) ? 0 : DIRECTION_CENTER_ROW | DIRECTION_BOTTOM_ROW;
}
...
int ObjMnts2::GetPassable( int icn, uint32_t index )
{
    ...
    return ARRAY_COUNT_END( disabled1 ) != std::find( disabled1, ARRAY_COUNT_END( disabled1 ), index ) ? 0 : DIRECTION_CENTER_ROW | DIRECTION_BOTTOM_ROW;
}
...
int ObjMnts1::GetActionObject( uint32_t )
{
    return MP2::OBJ_ZERO;
}

int ObjMnts2::GetActionObject( uint32_t )
{
    return MP2::OBJ_ZERO;
}
...
{
    const uint8_t shadows1[] = {0, 5, 11, 17, 21, 26, 32, 38, 42, 45, 49, 52, 55, 59, 62, 65, 68, 71, 74, 75, 79, 80};
    return ARRAY_COUNT_END( shadows1 ) != std::find( shadows1, ARRAY_COUNT_END( shadows1 ), index );
}

bool ObjMnts2::isShadow( uint32_t index )
{
    const uint8_t shadows2[] = {0, 5, 11, 17, 21, 26, 32, 38, 42, 46, 47, 53, 57, 58, 62, 68, 72, 75, 79, 82, 85, 89, 92, 95, 98, 101, 104, 105, 109, 110};
