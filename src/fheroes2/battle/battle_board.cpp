...
int32_t Battle::Board::GetIndexAbsPosition( const Point & pt ) const
{
...
bool Battle::Board::isImpassableIndex( int32_t index )
{
    const Cell * cell = Board::GetCell( index );
    return !cell || !cell->isPassable1( true );
}

bool Battle::Board::isBridgeIndex( int32_t index, int color )
{
    const Bridge * bridge = Arena::GetBridge();
...
bool Battle::Board::isMoatIndex( int32_t index, int color )
{
    switch ( index ) {
    case 7:
    case 18:
...
bool Battle::Board::isValidMirrorImageIndex( int32_t index, const Unit * troop )
{
    if ( troop == NULL )
        return false;
