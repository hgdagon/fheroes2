...
Battle::Unit::Unit( const Troop & t, int32_t pos, bool ref )
    : ArmyTroop( NULL, t )
    , animation( id )
    , uid( World::GetUniq() )
    , hp( t.GetHitPoints() )
...
void Battle::Unit::SetPosition( int32_t pos )
{
    if ( position.GetHead() )
...
uint32_t Battle::Unit::GetUID( void ) const
{
    return uid;
}
...
uint32_t Battle::Unit::GetShots( void ) const
{
    return shots;
}
...
StreamBase & Battle::operator<<( StreamBase & msg, const Unit & b )
{
    return msg << b.modes << b.id << b.count << b.uid << b.hp << b.count0 << b.dead << b.shots << b.disruptingray << b.reflect << b.GetHeadIndex()
               << ( b.mirror ? b.mirror->GetUID() : static_cast<uint32_t>( 0 ) ) << b.affected << b.blindanswer;
}
...
Point Battle::Unit::GetCenterPoint() const
{
    ...
    const int32_t centerY = pos.y + pos.h + sprite.y() / 2 - 10;
