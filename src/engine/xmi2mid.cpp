...
{
    uint8_t c1 = delta & 0x0000007F;
    uint8_t c2 = ( delta & 0x00003F80 ) >> 7;
    uint8_t c3 = ( delta & 0x001FC000 ) >> 14;
    uint8_t c4 = ( delta & 0x0FE00000 ) >> 21;
...
struct GroupChunkHeader
{
    ...
    GroupChunkHeader( uint32_t id, uint32_t sz, uint32_t tp )
        : ID( id )
        , length( sz )
        , type( tp )
    {}
...
struct XMIData
{
    ...
    XMIData( const std::vector<uint8_t> & buf )
    {
        StreamBuf sb( buf );
